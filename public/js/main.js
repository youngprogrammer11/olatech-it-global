...
async function loadCustomServices() {
  try {
    ...
    const cards = data.data.map((s, i) => {
      ...
      return `
        <div class="svc-card reveal">
          <span class="svc-index">${num}</span>
          <div class="svc-icon-wrap">${iconSvg}</div>
          <h3 class="svc-title">${s.title}</h3>
          <p class="svc-desc">${s.description}</p>
          ${tags ? `<div class="svc-tags">${tags}</div>` : ''}
        </div>`;
    }).join('');
    ...
    grid.querySelectorAll('.svc-card.reveal:not(.in)').forEach(el => revealObs.observe(el));

  } catch (err) {
    console.log('No custom services to load');
  }
}
